...
class RecursiveSync(object):
    ...
    def execute(self):
        newFolderPath = self._make_unicode(self.folderPath)
        if not os.path.exists(self._get_os_encoded_path(newFolderPath)):
        ...
        while filesRemain:
            filesRemain = 0
            for root, dirs, files in os.walk(self._get_os_encoded_path(newFolderPath)):
                root = self._make_unicode(root)
                for f in files:
                    f = self._make_unicode(f)
                    if f.endswith((u'.cloudf', u'.cloudf-dev')) or (f.endswith((u'.cloud', u'.cloud-dev')) and not self.noDownload):
                        ...
                        if not success:
                            ...
                            sys.exit(1)
            time.sleep(3)
        self._output_message('{}\n'.format(u"Done with recursive sync of " + newFolderPath))
        return True
    ...
    def _get_os_encoded_path(self, path):
        path = self._make_unicode(path)
        if sys.platform.startswith('win32'):
            path = u'\\\\?\\' + path
        else:
